...
import getProducts from '@salesforce/apex/SSG_ProductSelectorHelper.getProducts';
import getRelatedOptions from '@salesforce/apex/SSG_ProductSelectorHelper.getRelatedOptions';
import getPicklistsValues from '@salesforce/apex/SSG_ProductSelectorHelper.getPicklistsValues';
import importLineItem from '@salesforce/apex/SSG_ProductSelectorHelper.importLineItem';
import { NavigationMixin } from 'lightning/navigation';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import OPPORTUNITY_PRODUCT_OBJECT from '@salesforce/schema/OpportunityLineItem';
import { getObjectInfo } from 'lightning/uiObjectInfoApi';
import getAccessLevel from '@salesforce/apex/SSG_ProductSelectorHelper.getAccessLevel';
...
export default class SSGProductSelectorPageOne extends NavigationMixin(LightningElement){
		...
		.then(data => {
			if(!data.HasEditAccess){
				const event = new ShowToastEvent({
					title: 'Access Denied',
					message: 'You are not the Opportunity Owner',
	...
	renderedCallback(){
		console.log('TreeGrid -> ', this.treeGridData)
		try{
			this.expandRows();
		} catch {
			console.log('HEHEHE')
    ...
    getStateParameters(currentPageReference) {
       if (currentPageReference) {
		  ...
		  this.OptyName = currentPageReference.state.c__OptyName;
		  this.OptyOwner = currentPageReference.state.c__OptyOwner;
		  this.OpportunityBU = currentPageReference.state.c__BU;
	...
	async getFilteredProducts(){
		...
		.then((result) => {
			...
			this.productSelectionData.forEach(product => {
				if(product.Catalog__c === 'OUT'){
					product['rowClass'] = 'slds-is-selected';
					console.log('OUT')
				} else {
					product = {...product, rowClass : 'not-highlighted-row'}
				}
			})
			console.log(this.productSelectionData);
			*/
	...
	optionSelectedHandler(event){
		...
		//this.currentOptions = [...this.currentOptions, ...event.detail.selectedRows.filter(row => !this.optionsMap[row.Id].hasOwnProperty('disabled'))];
	...
	checkImportOK(){
		...
		console.log('IMPORT LIST', this.importList);
